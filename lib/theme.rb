class Theme
  ...
  attr_accessor :name, :title, :description, :preview_image, :site
  ...
  def description
    if @description_html.nil?
      @description_html = RedCloth.new(File.read( File.join(Theme.path_to_theme(site, name), "about.markdown") )).to_html(:markdown, :textile) rescue "#{title}"
    end
    @description_html
  end
...
private 
  ...
  def self.path_to_theme(site, theme)
    File.join(themes_root, site, theme)
  end
